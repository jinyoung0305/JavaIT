...
public class DBConnection1 {
	public static void main(String[] args) {
		...
		// 프로젝트 속성 -> Java Build Path -> Libraries -> Classpath -> Add External JARs.. -> mqsql-connector-java-8.0.31
...
class DBConn {
	...
	public DBConn() {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");			// 5버전 : com.mysql.jdbc.Driver
	...
	public void selectAll() {
		...
		try {
			...
			rs = stmt.executeQuery(sql);				// executeQuery() : 쿼리 날리기 (sql 작성문 전송)
			while(rs.next()) {
				int id = rs.getInt(1);
